    ...
    for x in range(int(coords[0]) - 1, int(coords[0]) + 2):
        for y in range(int(coords[1]) - 1, int(coords[1]) + 2):
            for z in range(int(coords[2]) - 1, int(coords[2]) + 2):
                for w in range(int(coords[3]) - 1, int(coords[3]) + 2):
                    string = "".join([str(x), ",", str(y),",", str(z),",", str(w)])
...
def coordsAround(coord):
    x, y, z, w = strCoord2intCoord(coord)
    out = []
    for i in range(x-1, x+2):
        for j in range(y-1, y+2):
            for k in range(z-1, z+2):
                for l in range(w-1, w+2):
                    out.append("".join([str(i), ",", str(j), ",", str(k),  ",", str(l)]))
    return out
